...
for i = 1:numel(outcomes)
    ...
    if ttype < 3
        ...
        behavior(i,5) = find(abs(frames-ponset) == min(abs(frames-ponset)),1,'first'); % frame of piezo
...
figure, hold on
plot(amp_levels_volts,phit,'ok')
mod_fit = fit(amp_levels_volts,phit,'logistic');
p25 = interp1(mod_fit((amp_levels_volts(1):0.01:amp_levels_volts(end))'),(amp_levels_volts(1):0.01:amp_levels_volts(end))', 0.25, 'linear');
p50 = interp1(mod_fit((amp_levels_volts(1):0.01:amp_levels_volts(end))'),(amp_levels_volts(1):0.01:amp_levels_volts(end))', 0.5, 'linear');
p75 = interp1(mod_fit((amp_levels_volts(1):0.01:amp_levels_volts(end))'),(amp_levels_volts(1):0.01:amp_levels_volts(end))', 0.75, 'linear');
plot((amp_levels_volts(1):0.01:amp_levels_volts(end))',mod_fit((amp_levels_volts(1):0.01:amp_levels_volts(end))'),'b')

line([0 p25],[0.25 0.25],'Color',[0.6 0.6 0.05])
line([0 p50],[0.5 0.5],'Color',[1 0.5 0])
line([0 p75],[0.75 0.75],'Color',[1 0 0])

line([p25 p25],[0 0.25],'Color',[0.6 0.6 0.05])
line([p50 p50],[0 0.5],'Color',[1 0.5 0])
line([p75 p75],[0 0.75],'Color',[1 0 0])
